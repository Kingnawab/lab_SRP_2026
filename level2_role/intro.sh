#!/usr/bin/env bash
# Animated hacker-style intro for Level 2.
# Usage:  ./intro.sh
...
printf '%s\n' "$COLOR_RESET"
sleep 0.3
printf '%s        [ CSE 29 :: LEVEL 2 :: ROLE UPGRADE ]%s\n\n' "$COLOR_MAGENTA_BOLD" "$COLOR_RESET"
sleep 0.4

printf '%s' "$COLOR_CYAN"
type_out "  > authenticating as student.................... ok" 0.015
type_out "  > scanning return address on stack............. found" 0.015
printf '%s\n' "$COLOR_RESET"
sleep 0.2

printf '%s' "$COLOR_GREEN_BOLD"
type_out "  psst... Level 1 only changed data. Level 2 changes where the CPU goes next."
type_out "  You are logged in as 'student'. Hijack the saved return address."
printf '%s\n' "$COLOR_RESET"
sleep 0.2
